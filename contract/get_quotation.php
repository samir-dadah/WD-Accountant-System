<?php defined('MR') or exit('No direct script access allowed'); ?>
<!DOCTYPE html>
...
<body>
    ...
    <header>
        ...
        <table>
            <tr>
                <td style="width: 60%;">
                    <p style="font-size:26px;font-weight:700;padding-top:24px;"><?php echo get_l('TEXT_QUOTATION') . ' : ' . $data['quotation_data']['quotation_id'] . ' - ' . date('y', strtotime($data['quotation_data']['quotation_date'])); ?></p>
                </td>
                <td style="width: 40%;padding-left:42.5px;padding-top:3px;line-height:1.5;">
                    <div style="display: inline-block;margin:0 auto;">
                        ...
                        <?php echo get_l('INVOICE_DATE_TEXT') . ' : ' . date('d.m.Y', strtotime($data['quotation_data']['quotation_date'])); ?>
                    </div>
                </td>
            </tr>
        </table>
    ...
    <main>
        ...
        <table style="margin-top:-8px;" width="100%" class="table text-center product_table " cellpadding="0" cellspacing="0">
            <tr style="background-color:rgba(243, 243, 243); padding-top: 4px !important;">
                ...
                <th align="center"><?php echo get_l('PRICE_TXT'); ?></th>
                <th align="center" width="25%"><?php echo get_l('TOTAL_TXT'); ?></th>
            ...
            ?>
                <tr>
                    ...
                    <td align="center" style="text-align: right;padding-right:65px;">
                        <?php echo price_format($product["price"]) . ' ' . $data['ccsympol']; ?>
                    </td>
                    <td style="text-align: right;padding-right:45px;">
                        <?php echo price_format($actual_amount) . ' ' . $data['ccsympol']; ?>
                    </td>
                </tr>
            <?php } ?>
        </table>
        ...
        <table>
            ...
            <tr>
                <td></td>
                <td></td>
                <td style="text-align: right; "><?php echo get_l('NET_TOTAL_TXT'); ?>
                </td>
                <td style="text-align: right;padding-right:45px;">
                    <?php echo price_format($total_actual_amount) . '  ' . $data['ccsympol'] ?>
                </td>
            </tr>
            <tr>
                <td></td>
                <td></td>
                <td style="text-align: right; "><?php echo get_l('TAX_TXT'); ?>
                </td>
                <td style="text-align: right;padding-right:45px;">
                    <?php echo price_format($total_tax_amount) . '  ' . $data['ccsympol'] ?>
                </td>
            </tr>
            <tr>
                <td></td>
                <td></td>
                <td style="text-align: right; "><?php echo get_l('SHIPPING_TXT'); ?>
                </td>
                <td style="text-align: right;padding-right:45px;">
                    <?php echo price_format($data['quotation_data']['shipping_cost']) . '  ' . $data['ccsympol'] ?>
                </td>
            </tr>
            <tr>
                <td></td>
                <td></td>
                <td style="text-align: right; "><?php echo get_l('FIRST_PAYMENT_TXT'); ?>
                </td>
                <td style="text-align: right;padding-right:45px;">
                    <?php echo price_format($data['quotation_data']['first_payment']) . '  ' . $data['ccsympol'] ?>
                </td>
            </tr>
            <tr>
                <td></td>
                <td></td>
                <td style="text-align: right; "><?php echo get_l('DISCOUNT_TXT'); ?>
                </td>
                <td style="text-align: right;padding-right:45px;">
                    <?php echo price_format($data['quotation_data']['discount_total']) . '  ' . $data['ccsympol'] ?>
                </td>
            </tr>
            <tr>
                <td></td>
                <td></td>
                <td style="text-align: right; "><?php echo get_l('GENERAL_TOTAL_TXT'); ?>
                </td>
                <td style="text-align: right;padding-right:45px;">
                    <?php

                    $total = $total - $data['quotation_data']['discount_total'];
                    $total = $total + $data['quotation_data']['shipping_cost'];
                    $total = $total - $data['quotation_data']['first_payment'];

                    echo price_format($total) . '  ' . $data['ccsympol'] ?>
                </td>
            </tr>
            <tr>
                <td colspan="2" style="text-align: left;"> <?php echo get_l('PAYMENT_METHOD_TXT') . ' ' . $data['payment_status']; ?></td>
                <td></td>
                <td></td>
            </tr>
            <tr>
                <td colspan="4" style="text-align: left;font-size:12px;">
                    <h3 style="font-weight: 700 !important;"><?php echo get_l('TEXT_QUOTATION_VALIDITY_TITLE'); ?></h3>
                    <p><?php echo get_l('TEXT_QUOTATION_VALIDITY_SUBTITLE'); ?></p>
                    <h3><?php echo get_l('TEXT_QUOTATION_CONDITION_TITLE'); ?></h3>
                    <p><?php echo get_l('TEXT_QUOTATION_CONDITION_SUBTITLE'); ?></p>
                </td>
            </tr>
        </table>
        <?php if ($data['quotation_data']['comment'] !== '') { ?>
            <table>
                <tr>
                    <td colspan="4" style="text-align: left;font-size:12px;">
                        <?php echo $data['quotation_data']['comment']; ?>
                    </td>
                </tr>
            </table>
        <?php } ?>
